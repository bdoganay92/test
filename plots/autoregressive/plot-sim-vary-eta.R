...
jpeg(file.path(path.plots,this.folder, "sim_vary_eta/eos_means.jpg"), width = 800, height = 800)
...
for(i in 1:10){
  ...
  axis(1, at = seq(0, 0.55, 0.10), cex.axis = 2.5, lwd = 5)
  axis(2, at = seq(0, 1, 0.20), cex.axis = 2.5, lwd = 5)
  lines(dat.plot$eta, dat.plot$power.diff.eos.means, lty = 2, lwd = 3, col = palette[i])
  points(dat.plot$eta, dat.plot$power.diff.eos.means, pch = 21, bg = palette[i], col = "black", cex = 3)
...
for(i in 1:10){
  text(x = 0.47, y = all.yloc[i], paste("delta=", round(all.deltaQ[i], 2),sep=""), col = palette[i], cex = 1.8, adj = 0)
}
...
jpeg(file.path(path.plots,this.folder, "sim_vary_eta/AUC.jpg"), width = 800, height = 800)
...
for(i in 1:10){
  ...
  axis(1, at = seq(0, 0.55, 0.10), cex.axis = 2.5, lwd = 5)
  axis(2, at = seq(0, 1, 0.20), cex.axis = 2.5, lwd = 5)
  lines(dat.plot$eta, dat.plot$power.diff.AUC, lty = 2, lwd = 3, col = palette[i])
  points(dat.plot$eta, dat.plot$power.diff.AUC, pch = 21, bg = palette[i], col = "black", cex = 3)
