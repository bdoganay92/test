...
for(idx.i in 1:length(list.input.rho)){
  ...
  for(idx.j in 1:length(list.input.means)){
    ...
    for(idx.k in 1:length(list.input.n4)){
      ...
      power.diff.eos.means$idx.n4 <- idx.k
      power.diff.change.score$idx.n4 <- idx.k
      ...
      coverage.diff.eos.means$idx.n4 <- idx.k
      coverage.diff.change.score$idx.n4 <- idx.k
...
# Save workspace
###############################################################################
save.image(file = file.path(path.output_data, use.working.corr, "fixedN-curve-03.RData"))
