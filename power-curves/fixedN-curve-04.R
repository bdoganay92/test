...
###############################################################################
input.N <- 300
list.input.n4 <- as.list(seq(0,120,by=30))
collect.power <- list()
...
for(idx.i in 1:length(list.input.rho)){
  ...
  for(idx.j in 1:length(auc.list.input.means)){
    ...
    for(idx.k in 1:length(list.input.n4)){
      input.n4 <- list.input.n4[[idx.k]]
      
      source(file.path(path.code,"calc-power.R"))
      power.diff.AUC$idx.input.means <- idx.j
      power.diff.AUC$idx.input.n4 <- idx.k
      ...
      coverage.diff.AUC$idx.input.means <- idx.j
      coverage.diff.AUC$idx.n4 <- idx.k
      tmp.coverage <- list(AUC = coverage.diff.AUC[coverage.diff.AUC$pair==this.pair,])
...
# Save workspace
###############################################################################
save.image(file = file.path(path.output_data, use.working.corr, "fixedN-curve-04.RData"))
