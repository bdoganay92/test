...
.df.vary.params <- expand.grid(rho = 0.90, N = seq(100,500,25))

.df.vary.params$power.eos.means.method02 <- NA_real_
.df.vary.params$power.AUC.method02 <- NA_real_

###############################################################################
# Employ Method 2 in Power Calculation
###############################################################################

.this.folder.alternative <- "sim_study_supp/sim_size_test/high_zeros"
...
for(.idx.vary.params in 1:nrow(.df.vary.params)){
  ...
  .df.vary.params[.idx.vary.params,"power.eos.means.method02"] <- power.diff.eos.means
  .df.vary.params[.idx.vary.params,"power.AUC.method02"] <- power.diff.AUC
...
save(power.table, file = file.path(.path.output_data, .this.folder, "power_method_02.RData"))
