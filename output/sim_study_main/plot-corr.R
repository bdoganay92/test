...
##########################################################################################
this.folder <- "sim_study_main/sim_results_null/truth"
...
# Loading this RData file will place collect.correlation.tau in the global environment
##########################################################################################
this.folder <- "sim_study_main/sim_results_null/above_truth"
load(file.path(path.output_data, this.folder,"correspondence_between_rho_and_tau.RData"))

# Begin drawing lines corresponding to tau.mean
lines(x = collect.correlation.tau$datagen.params.rho, y = collect.correlation.tau$tau.mean, type="o", col="darkgoldenrod", lwd=2, lty=3)
...
# Begin drawing lines corresponding to tau.mean
lines(x = collect.correlation.tau$datagen.params.rho, y = collect.correlation.tau$tau.mean, type="o", col="tomato", lwd=2, lty=3)
...
legend("topleft", 
       c("Under H0: truth", "Under H0: below truth", "Under H0: above truth","Under Ha", "45 degree line"), 
       col=c("darkgreen","darkblue","darkgoldenrod","tomato","black"), 
       lty=c(3,3,3,3,2), 
       lwd=c(3,3,3,3,1))
