...
# -----------------------------------------------------------------------------
# Vary inputs here
input.N <- c(200,500,2000)  # Total no. of individuals
input.rho <- c(0.10,0.95)  # Dependence parameter
...
# -----------------------------------------------------------------------------
source(file.path(path.code,"calc-bias.R"))

list.all.est.bias.betas$bias.betas %>% 
  group_by(datagen.params.N, datagen.params.rho) %>%
  summarise(mean.bias=mean(est.bias)) %>% 
  write.csv(., file.path(path.output_data,"mean.bias.betaj.csv"))

list.all.est.bias.means$bias.u.plusplus %>% 
  group_by(datagen.params.N, datagen.params.rho) %>%
  summarise(mean.bias=mean(est.bias)) %>% 
  write.csv(., file.path(path.output_data,"mean.bias.mut.plusplus.csv"))

list.all.est.bias.means$bias.u.minusplus %>% 
  group_by(datagen.params.N, datagen.params.rho) %>%
  summarise(mean.bias=mean(est.bias)) %>% 
  write.csv(., file.path(path.output_data,"mean.bias.mut.minusplus.csv"))

list.all.est.bias.diffs.eos.means$bias.diff.eos.means.plusplus.minusplus %>% 
  write.csv(., file.path(path.output_data,"bias.diff.eos.means.plusplus.minusplus.csv"))

list.all.est.bias.diffs.AUC$bias.diff.AUC.plusplus.minusplus  %>% 
  write.csv(., file.path(path.output_data,"bias.diff.AUC.plusplus.minusplus.csv"))
...
list.bias.est.stderr.diff.eos.means$bias.est.stderr.diff.eos.means.plusplus.plusminus %>% 
  write.csv(., file.path(path.output_data,"bias.stderr.diff.eos.means.plusplus.minusplus.csv"))
list.bias.est.stderr.diff.AUC$bias.est.stderr.diff.AUC.plusplus.plusminus %>%
  write.csv(., file.path(path.output_data,"bias.stderr.diff.AUC.plusplus.minusplus.csv"))

list.coverage.diff.eos.means$coverage.diff.eos.means.plusplus.plusminus %>% 
  write.csv(., file.path(path.output_data,"coverage.diff.eos.means.plusplus.minusplus.csv"))
list.coverage.diff.AUC$coverage.diff.AUC.plusplus.plusminus %>%
  write.csv(., file.path(path.output_data,"coverage.diff.AUC.plusplus.minusplus.csv"))
